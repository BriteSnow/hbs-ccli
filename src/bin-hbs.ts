...
interface HbsConfigEntry extends ProcessConfig {
...
async function run(argv: ParsedArgs) {
	...
	const confFile = argv.c ?? 'hbs.config.js';
	...
	for (const entry of entries) {
		...
		const processOpts: ProcessConfig = { input, output };
...
async function parseConfFile(confFile: string): Promise<HbsConfigEntry[]> {
	const exists = await pathExists(confFile);
	if (!exists) {
		throw new Error(`ERROR - hbs-cli - Cannot find file ${confFile}`);
	}
	...
	const entries: HbsConfigEntry[] = deepClone(confFileEntries);
...
function assertConfigEntries(arr: any[]): asserts arr is HbsConfigEntry[] {
	for (const obj of arr) {
		if (obj.input == null || obj.output == null) {
			throw new Error('ERROR - hbs-cli - invalid entry, each entry must have .input (string | string[]) and .output (string) properties')
		}
	}
